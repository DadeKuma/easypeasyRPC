...
public class RpcManager {
    ...
    public RpcManager(MethodPerformer performer) {
        this(performer, new MethodList());
    }

    public RpcManager(MethodPerformer performer, MethodList methodList) {
    ...
    public JsonElement parseRequest(String stringRequest){
        ...
        try{
            if(stringRequest == null || stringRequest.isEmpty()) {
                RpcResponse response = new RpcResponse(ErrorType.INVALID_REQUEST,null, RPC_VERSION);
                return gson.toJsonTree(response);
            }
            jsonElement = parser.parse(stringRequest);
        }
        catch (JsonSyntaxException ex){
            RpcResponse response = new RpcResponse(ErrorType.PARSE_ERROR,null, RPC_VERSION);
            return gson.toJsonTree(response);
        }
    ...
    private JsonElement parseBatchRequest(JsonElement batchRequest){
        ...
        if(jsonArray.size() < 1){
            return gson.toJsonTree(new RpcResponse(ErrorType.INVALID_REQUEST, null, RPC_VERSION));
        }
        ...
        for(JsonElement element : jsonArray){
            ...
            if(!element.isJsonObject())
                responses.add(gson.toJsonTree(new RpcResponse(ErrorType.INVALID_REQUEST, null, RPC_VERSION)));
            else{
    ...
    private RpcResponse compileResponse(com.github.dadekuma.easypeasyrpc.resource.RpcRequest request){
        ...
        if(requestMethod == null || requestMethod.isEmpty())
            return new RpcResponse(ErrorType.INVALID_REQUEST, null, RPC_VERSION);
        if(!methodList.getMethods().containsKey(requestMethod))
            return new RpcResponse(ErrorType.METHOD_NOT_FOUND, request.getId(), RPC_VERSION);
        ...
        try{
            ...
            if((params == null && !methodHasZeroParams) || !hasSameParams)
                return new RpcResponse(ErrorType.INVALID_PARAMS, request.getId(), RPC_VERSION);
            ...
            e.printStackTrace();
            return new RpcResponse(ErrorType.INTERNAL_ERROR, request.getId(), RPC_VERSION);
        }
        catch (RpcException e){
